...
impl<T: InvokeUiSession> Remote<T> {
    ...
    pub async fn io_loop(&mut self, key: &str, token: &str, round: u32) {
        ...
        .await
        {
            Ok(((mut peer, direct, pk, kcp, stream_type, endpoint), (feedback, rendezvous_server))) => {
                self.handler
                    ...
                    .set_connected();
                self.handler
                    .set_connection_type(peer.is_secured(), direct, stream_type, &endpoint); // flutter -> connection_ready
